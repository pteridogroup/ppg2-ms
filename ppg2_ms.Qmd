...
When PPG joined WFO as a TEN, data for ferns and lycophytes were already in Rhakhis, but the classification scheme was based on the World Checklist of Vascular Plants [@govaertsWorldChecklistVascular2021], which which reflected the views of its editors rather than the larger community and deviated substantially from PPG I [-@pteridophytephylogenygroupi2016communityderived].
...
Whenever a new version is released, the classification on our website (<https://pteridogroup.github.io/>) will be immediately and automatically updated.
...
Note that although WFO classification is derived directly from PPG, it has its own release schedule (twice yearly, on the solstices) and may not always include the most recent changes.
...
Consistent with other higher level classifications (e.g., Ruggiero et al. 2015), two classes are recognized: Lycopodiopsida (lycophytes) and Polypodiopsida (ferns; @fig-tree).
...
Our recognition of `r n$subclass$ppg2` subclasses is also unchanged from PPG I.
...
The number of taxa at the level of species and below (>13,000 accepted species) precludes publishing those names here, but the full classification (extending to infraspecific ranks) is available on our website (<https://pteridogroup.github.io/>) and WFO (<https://www.worldfloraonline.org/>).

## Discussion

**An open, community-based approach to taxonomy.---** PPG is unique in its open, community-based approach to taxonomy.
While other community efforts have been established [e.g., Angiosperm Phylogeny Group, Legume Phylogeny Working Group, Grass Phylogeny Working Group; @leroux2022expert], to our knowledge, no comparable classification working group exists that makes its discussions and results transparent and publicly available.
...
This reflects the technological progress made in next-generation DNA sequencing, which is helping to clarify relationships that were intractable with morphological approaches alone.
...
Taxa at the rank of class to subfamily presented in the order they appear in @fig-tree; genera presented alphabetically.
