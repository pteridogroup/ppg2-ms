...
Our understanding of fern and lycophyte relationships has been revolutionized by molecular phylogenetics, which demonstrated that horsetails and whisk ferns are nested within ferns and further that ferns are sister to seed plants and only distantly related to the lycophytes [@pryer2001horsetails].
...
Accordingly, the PPG community provides revised taxonomic system for ferns and lycophytes.
...
Additionally, proposals were required to: (1) implement a change relative to PPG I [-@pteridophytephylogenygroupi2016communityderived]; (2) only include names that have been validly published under the International Code for Algae, Fungi, and Plants; (3) involve taxa at the genus level or higher.
...
If the nomenclatural proposal is rejected, we will update the PPG classification accordingly.
...
Given that there are >13,000 accepted pteridophyte species names, it is not feasible to implement the same discussion and voting protocol for ranks at species and below that we applied to ranks at genus and above.
...
All data and code used to generate this manuscript is available at <https://github.com/pteridogroup/ppg2-ms>.
...
We live in an age where the generation of data exceeds the ability of any single individual to comprehensively track new insights across such a large and heterogeneous group as pteridophytes.
...
This is driven primarily by a few (sub)families: Thelypteridaceae (`r nchild$Thelypteridaceae$n_genera` vs. 30), Grammitidoideae (`r nchild$Grammitidoideae$n_genera` vs. 33), Cheilanthoideae (`r nchild$cheilanthoideae$n_genera` vs. 23), and Blechnaceae (`r nchild$Blechnaceae$n_genera` vs. 18).
