...
csl: https://www.zotero.org/styles/taxon
execute: 
  echo: false
  message: false
  warning: false
...
authors <- read_csv("data/ppg2_author_list.csv") |>
  ...
  arrange(order, last_name) %>%
  mutate(
    other_author = str_detect(name, "Hassler|Nitta|Rothfels|Schneider|Schuettpelz", negate = TRUE),
    auth_num = seq(nrow(.)),
...
Recommended citation: PPG II (2026). This project was organized by Joel H. Nitta\*^1^, Michael Hassler^2^, Carl Rothfels^3^, Harald Schneider^4^, and Eric Schuettpelz\*^5^; with additional contributions from (in alphabetical order): `r other_authors`
...
The most recent widely accepted classification for ferns and lycophytes is the Pteridophyte Phylogeny Group (PPG) I classification, published in 2016.
Since that time, many new names have been published and phylogenetic relationships have come into clearer focus.
...
Our understanding of fern and lycophyte relationships was revolutionized by molecular phylogenetics, which demonstrated that horsetails and whisk ferns are nested within ferns and further that ferns are sister to seed plants and only distantly related to the lycophytes [@pryer2001horsetails].
...
PPG I was remarkable for its community-based approach, which sought to obtain consensus among fern and lycophyte experts and included nearly 90 co-authors.
...
Accordingly, the PPG community aimed to produce a revised taxonomic system for ferns and lycophytes.

## Methods

To maximize participation in taxonomic discussions and ensure that justifications for taxonomic decisions were openly available, we implemented a system that is, to our knowledge, unique amongst taxonomic communities.
...
Additionally, proposals were required to: (1) implement a change relative to PPG I [-@pteridophytephylogenygroupi2016communityderived]; (2) only include names that have already been validly published under the International Code for Algae, Fungi, and Plants (hereafter, "the Code"); (3) involve taxa at the genus level or higher.
...
Because such proposals may not be voted on by the ICPN for several years, we allowed such names to be voted on and approved by PPG.
...
Anyone could submit a proposal providing it met the taxonomic criteria and they agreed to our Code of Conduct; proposals were then subject to an open discussion period of at least one month.
...
The proposal and discussion process ran continuously, with a ballot issued monthly.
...
World Flora Online relies on a network of taxonomic experts, called Taxonomic Expert Networks (TENs), to curate the taxonomy of specific plant groups.
...
Furthermore, Rhakhis is available as a live, web-based database, so PPG members can collaboratively edit the data in realtime, with no need for synchronizing database files across multiple machines.

The initial version of the taxonomic data was based on a copy of the World Ferns database [@hassler2025world], which was imported into Rhakhis and edited to reflect accepted taxonomic proposals from our voting process.
...
By starting from the World Ferns classification [@hassler2025world], PPG II was "pre-populated" with these lower ranked taxa.
...
Given that there are >13,000 accepted species names, it is not feasible to implement the same discussion and voting protocol for ranks at species and below that we applied to ranks at genus and above.
Instead, we have established committees of specialists who are each responsible for reviewing and maintaining taxonomic data at the species level and below in Rhakhis for a particular pteridophyte taxon (or taxa), such that all taxa are accounted for.
...
This will greatly benefit stakeholders who need an up-to-date, standardized taxonomy for ferns and lycophytes, while simultaneously providing, through the versioning system, the ability to unambiguously reference a specific snapshot of the classification.
...
# Check that assessment of World Ferns genera not in PPG II is correct
gen_in_wf_not_ppg_2 <- c(
  "Brownseya",
  "Davallodes",
  "Orthiopteris",
  "Physematium",
...
ppg_2_accepted_gen <- ppg |>
  filter(taxonRank == "genus", taxonomicStatus == "accepted") |>
  pull(scientificName)

assertthat::assert_that(
  !any(gen_in_wf_not_ppg_2 %in% ppg_2_accepted_gen),
  msg = "Statement about World Ferns genera not in PPG II is incorrect"
)
```
...
The resulting genus-and-above PPG II classification is presented below.

PPG II recognizes one genus not included in World Ferns v25.12 (*Thysanosoria* A.Gepp) and lacks five genera recognized there (*Brownseya* Li Bing Zhang, L.D.Sheph., D.K.Chen, X.M.Zhou & H.He, *Davallodes* (Copel.) Copel., *Orthiopteris* Copel., *Physematium* Kaulf., and *Podosorus* Holttum).
...
The community approach allows us to distribute the workload and stay abreast of the latest advances in the systematics of these plants, which will lead, we hope, to a continuously updated, maximally useful taxonomy.
...
The general trend, in comparison to PPG I, has been an increase in genus number.
This is driven primarily by a few (sub)families: Thelypteridaceae (`r nchild$Thelypteridaceae$n_genera` vs. 30), Grammitidoideae (`r nchild$Grammitidoideae$n_genera` vs. 33), and Blechnaceae (`r nchild$Blechnaceae$n_genera` vs. 18).
...
In addition, there are some relatively deep relationships that have still not been adequately resolved, especially due to conflicts between inferences derived from nuclear versus plastid datasets, as indicated by the dashed lines in @fig-tree and summarized in @tbl-uncertain.

One advantage of using rolling, monthly voting is that, going forward, a taxonomic proposal may be submitted at any time, and proposals that did not pass may be revised and/or revisited in the future.
...
Circumscriptions and synonymy are generally not shown, although we do account for all names recognized in PPG I (2016); where applicable, such names are included as synonyms if not recognized herein.
...
#|   (accepted or synonym). Hybrids not included in species count.
#|   Complete synonymy was not presented in PPG I so this is not shown.
#|   Not every rank is used by every treatment; for example PPG II does not use tribe."
...
fig_tree_cap <- glue::glue("
  ...
  @nitta2022open recovered Salviniales sister to Cyatheales, but this has generally not been observed in other studies [e.g., @testo20164000species; @rothfels2015evolutionary; @du2022structural; @schuettpelz2007fern], so the consensus topology is shown instead.
  ") |>
  as.character() |>
  str_remove_all("//\n") |>
  str_squish()
...
We thank Allan Elliot and Roger Hyam for technical support in transitioning to the World Flora Online database, and World Flora Online for its cooperation and recognition of PPG as a TEN.
