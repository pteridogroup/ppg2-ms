...
Most taxonomic changes were caused by the addition of new names, i.e., splitting (@tbl-changes), whereas a smaller number of changes stems from the subsumption of existing names, i.e., lumping (@tbl-changes).
...
Since PPG I [-@pteridophytephylogenygroupi2016communityderived], which included XX authors representing XX countries, the PPG community has grown to include `r n_authors` authors representing `r n_countries` countries.
...
In comparison to PPG I, the general trend has been an increase in the number of genera (@tbl-changes; Figure S1).
