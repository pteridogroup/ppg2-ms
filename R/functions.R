...
read_ppg_from_archive <- function(url) {
  ...
  download.file(url, temp_tar, mode = "wb", quiet = TRUE)

  # Extract the archive
  untar(temp_tar, exdir = temp_dir)

  # Find the data/ppg.csv file (accounting for archive directory
  # structure)
  extracted_files <- list.files(temp_dir, recursive = TRUE, full.names = TRUE)
  ppg_csv <- extracted_files[grep("data/ppg\\.csv$", extracted_files)]

  if (length(ppg_csv) == 0) {
    stop("Could not find data/ppg.csv in the archive")
  }

  # Read the CSV file
  result <- read_csv(ppg_csv[1], show_col_types = FALSE)
